...
export default function Address({ user }) {
  ...
  return (
    <div className="mt-6 rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-6 shadow-sm transition-colors duration-300">
      ...
      {address && !isEditing ? (
        <div className="rounded-lg border border-[var(--border)] transition-colors duration-300 p-4">
          ...
          <div className="mt-4 flex gap-2">
            ...
            <button onClick={handleDelete}
              className="rounded-lg border border-[var(--danger)] px-4 py-2 text-sm font-semibold text-[var(--danger)] transition hover:bg-[var(--danger)] hover:text-[var(--text-inverse)] transition-colors duration-300">
              Delete </button>
          </div>
        </div>
      ) : (
        <>
          ...
          <button
            onClick={handleSave}
            className="mt-4 flex items-center gap-2 rounded-lg bg-[var(--primary)] px-6 py-3 text-sm font-semibold text-[var(--text-inverse)] transition hover:opacity-90 transition-colors duration-300"
          >
